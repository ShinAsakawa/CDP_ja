...
class MultiHeadAttention(nn.Module):
	...
	def __init__(self,
				 ...
				 device:str=torch.device('cuda:0' if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu')
				):
		super().__init__()
		...
		self.W_q = nn.Linear(in_features=model_dim, out_features=model_dim, device=device)
		self.W_k = nn.Linear(in_features=model_dim, out_features=model_dim, device=device)
		self.W_v = nn.Linear(in_features=model_dim, out_features=model_dim, device=device)
		self.W_o = nn.Linear(in_features=model_dim, out_features=model_dim, device=device)
...
class PositionWiseFeedForward(nn.Module):
	...
	def __init__(self,
		...
		super().__init__()
		self.fc1 = nn.Linear(in_features=model_dim, out_features=ff_dim, device=device)
		self.fc2 = nn.Linear(in_features=ff_dim, out_features=model_dim, device=device)
...
class EncoderLayer(nn.Module):
	...
	def __init__(
		...
		super().__init__()
		self.self_attn = MultiHeadAttention(model_dim, num_heads, device=device)
		self.feed_forward = PositionWiseFeedForward(model_dim, ff_dim, device=device)
		self.norm1 = nn.LayerNorm(model_dim, device=device)
		self.norm2 = nn.LayerNorm(model_dim, device=device)
...
class DecoderLayer(nn.Module):
	...
	def __init__(
		...
		self.self_attn = MultiHeadAttention(model_dim, num_heads, device=device)
		self.cross_attn = MultiHeadAttention(model_dim, num_heads,device=device)
		self.feed_forward = PositionWiseFeedForward(model_dim, ff_dim, device=device)
		self.norm1 = nn.LayerNorm(model_dim, device=device)
		self.norm2 = nn.LayerNorm(model_dim, device=device)
		self.norm3 = nn.LayerNorm(model_dim, device=device)
		# self.norm1 = nn.LayerNorm(model_dim)
		# self.norm2 = nn.LayerNorm(model_dim)
